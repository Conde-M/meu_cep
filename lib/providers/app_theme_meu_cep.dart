...
import 'package:meu_cep/providers/text_theme_meu_cep.dart';

// Classe principal para gerenciar o tema do aplicativo
class AppThemeMeuCep extends InheritedWidget {
  const AppThemeMeuCep({
    Key? key,
...
// Métodos para definir o tema claro e escuro
  ...
  final ValueNotifier<MeuCepAppThemeSettings> settings;
...
// Métodos para definição de estilos de elementos de interface do usuário
  ...
  ListTileThemeData listTileTheme(ColorScheme colors) {
    return const ListTileThemeData().copyWith(
      ...
      leadingAndTrailingTextStyle: textThemeMeuCep().labelSmall,
      selectedTileColor: colors.primaryContainer,
      selectedColor: colors.secondary,
      style: ListTileStyle.list,
      textColor: colors.onSurfaceVariant,
      tileColor: colors.surfaceVariant,
      titleTextStyle: textThemeMeuCep().bodyLarge,
      subtitleTextStyle:
          textThemeMeuCep().bodyMedium!.copyWith(fontStyle: FontStyle.italic),
      shape: shapeMedium,
      enableFeedback: true,
    );
  }
  ...
  AppBarTheme appBarTheme(ColorScheme colorScheme) {
    return const AppBarTheme().copyWith(
      ...
      titleTextStyle: TextStyle(
        ...
        fontSize: textThemeMeuCep().titleLarge!.fontSize,
        fontFamily: textThemeMeuCep().titleLarge!.fontFamily,
        fontWeight: textThemeMeuCep().titleLarge!.fontWeight,
      ),
      surfaceTintColor: colorScheme.surfaceTint,
      color: colorScheme.surface,
      shadowColor: colorScheme.shadow,
      actionsIconTheme: IconThemeData(
        size: textThemeMeuCep().titleLarge!.fontSize,
        color: colorScheme.onSurface,
      ),
    );
  }
...
// Método para definição do tema do interruptor (switch)
  ...
  DialogTheme dialogTheme(ColorScheme colorScheme) {
    return const DialogTheme().copyWith(
      backgroundColor:
          adjustColor(colorScheme.brightness, colorScheme.surface, 45),
      titleTextStyle: textThemeMeuCep().headlineSmall!.copyWith(
          color: adjustColor(colorScheme.brightness, colorScheme.onSurface, 45),
          fontSize: textThemeMeuCep().headlineSmall!.fontSize),
      contentTextStyle: textThemeMeuCep().bodyMedium!.copyWith(
            color: adjustColor(
  ...
  FloatingActionButtonThemeData floatingActionButtonTheme(
    ...
    return const FloatingActionButtonThemeData().copyWith(
      extendedTextStyle: TextStyle(
        fontStyle: textThemeMeuCep().bodyLarge!.fontStyle,
        fontFamily: textThemeMeuCep().bodyLarge!.fontFamily,
        textBaseline: textThemeMeuCep().bodyLarge!.textBaseline,
    ...
    return ElevatedButtonThemeData(
      style: const ButtonStyle().copyWith(
        textStyle: MaterialStateProperty.all(
          TextStyle(
            fontFamily: textThemeMeuCep().labelLarge!.fontFamily,
            fontSize: textThemeMeuCep().labelLarge!.fontSize,
            fontWeight: textThemeMeuCep().labelLarge!.fontWeight,
            decorationThickness: 20,
          ),
        ),
        iconSize:
            MaterialStateProperty.all(textThemeMeuCep().labelLarge!.fontSize),
        enableFeedback: true,
      ),
    );
  }
...
// Método estático para obter uma instância de AppThemeImaco do contexto
  static AppThemeMeuCep of(BuildContext context) {
    return context.dependOnInheritedWidgetOfExactType<AppThemeMeuCep>()!;
  }

// Override para notificar quando as configurações do tema mudam
  @override
  bool updateShouldNotify(covariant AppThemeMeuCep oldWidget) {
    return oldWidget.settings != settings;
  }
}

// Classe para armazenar configurações do tema do aplicativo
class MeuCepAppThemeSettings {
...
class CustomColor {
  ...
  Color value(AppThemeMeuCep provider) {
...
class ThemeSettingChange extends Notification {
  ...
  final MeuCepAppThemeSettings settings;
}
